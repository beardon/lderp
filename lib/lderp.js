...
Lderp.prototype.bindAsUser = function (dn, password) {
...
Lderp.prototype.bindAsZombie = function () {
    throw new Error('Lderp#bindAsZombie must be overridden by subclass');
};

Lderp.prototype.buildDn = function (username)  {
    throw new Error('Lderp#buildDn must be overridden by subclass');
};

Lderp.prototype.buildLdapChangeObject = function (operation, modification) {
...
Lderp.prototype.buildUserEntry = function (options) {
    throw new Error('Lderp#buildUserEntry must be overridden by subclass');
};

Lderp.prototype._buildUserFilter = function (username) {
    return '(' + this.usernameAttribute + '=' + username + ')';
};

Lderp.prototype._buildFilterFromWhere = function (where) {
    var filters = [];
...
Lderp.prototype._checkBindStatus = function () {
    var self = this;
...
Lderp.prototype.checkStatus = function () {
    var start = moment();
...
Lderp.prototype.createUser = function (dn, options) {
    return this._client.addAsync(dn, this.buildUserEntry(options));
};

Lderp.prototype.deleteUser = function (dn) {
    return this._client.delAsync(dn);
};

Lderp.prototype.find = function (where) {
...
Lderp.prototype.findUser = function (username) {
...
Lderp.getPersonalNameByUsername = function (username) {
    var self = this;
...
Lderp.prototype._getPersonalNameByEntry = function (entry) {
    return (!!entry ? {first: entry.givenName, last: entry.sn} : null);
};

Lderp.prototype._search = function (filter, options) {
    options = options || {};
    var opts = {
...
Lderp.prototype.unbind = function () {
