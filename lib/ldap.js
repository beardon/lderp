...
Ldap.prototype._buildFilterFromWhere = function (where) {
    let filters = _.map(where, function (value, key) {
        return '(' + key + '=' + value + ')';
    });
    if (filters.length > 1) {
        return '(&' + filters.join('') + ')';
...
Ldap.prototype.find = function (where) {
    const filter = (_.isEmpty(where) ? this._buildUserFilter('*') : (!where.filter ? this._buildFilterFromWhere(where) : where.filter));
